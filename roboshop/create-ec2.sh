...
if [ -z "$1" ] || [ -z "$2" ]  ; then
    ...
    echo -e "Ex Usage : \n \t \t  bash create-ec2 componentName envName"
    exit 1
fi 
...
create_ec2() {
        ...
        IPADDRESS=$(aws ec2 run-instances --image-id ami-0c1d144c8fdd8d690 --instance-type t3.micro --security-group-ids sg-08212398e8c9c44b6 --tag-specifications "ResourceType=instance,Tags=[{Key=Name,Value=$COMPONENT-$ENV}]" | jq '.Instances[].PrivateIpAddress' | sed -e 's/"//g')

        echo -e  "*** Launching $COMPONENT-$ENV server **** "
        echo -e "Private IP Address of $COMPONENT-$ENV is $IPADDRESS"  
